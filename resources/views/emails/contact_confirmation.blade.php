...
<body>
    <div class="container">
        <div class="header">
            @php
            try {
                $imageData = base64_encode(file_get_contents(public_path('images/blackcat-logo.png')));
                $imageSrc = "data:image/png;base64," . $imageData;
            } catch (\Exception $e) {
            $imageSrc = "https://i.ibb.co/7wBv1FW/blackcat-logo.png";
            }
            @endphp
            <img src="{{ $imageSrc }}" 
             alt="BlackCat Logo" 
                                            ...
                                            <li class="desafio-item">
                                                @if(is_array($desafio) && isset($desafio['title']))
                                                    ...
                                                    @if(isset($desafio['recommendedSolution']))
                                                        <div class="desafio-solution">
                                                            Solución recomendada: {{ $desafio['recommendedSolution'] }}
                                                        </div>
                                                    @endif
                                                @else
                                                    <div class="desafio-title">
                                                        {{ is_string($desafio) ? $desafio : 'Desafío no especificado' }}
                                                    </div>
                                                @endif
                                            </li>
                                        @endforeach
